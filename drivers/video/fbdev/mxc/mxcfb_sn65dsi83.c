...
static int sn65dsi_write(u8 reg, u8 val)
{
	...
	dev_notice(&client->dev, "%s: write reg 0x%02x data 0x%02x", __func__, reg, val);
...
static int sn65dsi_read(u8 reg)
{
	...
	dev_notice(&client->dev, "%s: read reg 0x%02x data 0x%02x", __func__, reg, ret);
...
MODULE_AUTHOR("Ilya Ledvich <ilya@compulab.com>");
